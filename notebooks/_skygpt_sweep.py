...
if _mn.exists(): _MMU,_MSD=np.load(_mn); _MDIM=len(_MMU)
else: _MMU,_MSD=np.zeros(4,np.float32),np.ones(4,np.float32); _MDIM=4
_MGRID=1 if _MDIM==4 else int(round((_MDIM/3)**0.5))   # infer grid from descriptor length
_Hs=64;_cyx=_Hs//2;_r2s=(_Hs//4)**2;_yy2,_xx2=np.ogrid[:_Hs,:_Hs];_sunm=((_yy2-_cyx)**2+(_xx2-_cyx)**2)<=_r2s
def _mdesc(dx,dy,mag):
    if _MGRID<=1: return [dx.mean(),dy.mean(),mag.mean(),mag[_sunm].mean()]
    out=[]
...
for i in range(N_SKY):
    ...
    for j in range(1,16):
        ...
        dx,dy=f[...,0],f[...,1];mag=np.sqrt(dx*dx+dy*dy); Mlog[i,j]=_mdesc(dx,dy,mag)
